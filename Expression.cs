namespace Expression
{
    ...
    public class Expression
    {
        ...
        /// <summary>
        /// Maximum number of arguments in function call or in any list
        /// </summary>
        public int argnum;
        ...
        public string Evaluate(bool hex)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                if (hex)
                {
                    char c;
                    strarg = "";
                    for (long d = (long)args[i]; d != 0; d /= 16)
                    {
                        c = (char)(d % 16);
        ...
        protected int expressList(double[] args, ref int curpos, ref string lexeme, ref Lexeme type)
        {
            if (type == Lexeme.End) return 0;

            int i = 0;
            double d = expressBoolean(ref curpos, ref lexeme, ref type);
            args[i] = d; i++;
            ...
            while (!complete)
            {
                switch (lexeme)
                {
                    ...
                    case "and":
                        {
                            extractLexem(ref curpos, out lexeme, out type);
                            d = (d != 0 && expressRelational(ref curpos, ref lexeme, ref type) != 0) ? 1 : 0;
                            break;
                        }
                    ...
                    case "imp":
                        {
                            extractLexem(ref curpos, out lexeme, out type);
                            d = (d != 0 && expressRelational(ref curpos, ref lexeme, ref type) == 0) ? 0 : 1;
                            break;
                        }
        ...
        /// <param name="type">Type of current processing lexeme</param>
        /// <returns>Result of calculation</returns>
        protected int expressFactor(double[] args, ref int curpos, ref string lexeme, ref Lexeme type)
        {
            int retval;
            switch (lexeme)
            {
                case "(":
                    {
                        extractLexem(ref curpos, out lexeme, out type);
                        retval = expressList(args, ref curpos, ref lexeme, ref type);
                ...
                default:
                    args[0] = expressTerm(ref curpos, ref lexeme, ref type);
                    retval = 1;
                    break;
            }
            return retval;
                ...
                case "min":
                    {
                        ...
                        int n = expressFactor(args, ref curpos, ref lexeme, ref type);
                        if (n > 0) d = args[0];
                        else throw new ExpressException("Function expect at last 1 argument", curpos, lexeme);
                        for (int i = 1; i < n; i++)
                        {
                            if (args[i] < d) d = args[i];
                        }
                        return d;
                    }
                ...
                case "doz":
                    {
                        double[] args = new double[2];
                        int n = expressFactor(args, ref curpos, ref lexeme, ref type);
                        if (n != 2) throw new ExpressException("Function expect 2 argument", curpos, lexeme);
